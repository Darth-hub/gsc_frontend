    ...
    {data.map((item, index) => (
      ...
      >
        <td datakey={item.id} className="pl-4 pr-4 pt-2 flex items-center gap-3">
          <span className="text-xl font-medium">{item.sellerName}</span>
        </td>
        
        {[  
          { src: "https://res.cloudinary.com/dkgclbg58/image/upload/v1743510765/map_icon_vxyie2.png", text: item.pickUpLocation },
          { src: "https://res.cloudinary.com/dkgclbg58/image/upload/v1743510764/gmail_tlbzsq.png", text: item.sellerEmail },
          { src: "https://res.cloudinary.com/dkgclbg58/image/upload/v1743510765/garbage_clfuld.png", text: `${item.estimatedWeight} kg (expected by user)` }
        ].map((data, i) => (
          <td key={i} className="pl-4 pr-4 pt-2 flex items-baseline gap-3">
